...
const styles = StyleSheet.create({
  ...
  export function ClientsReport() {
    ...
    async function handleGeneratePDF(trip: Trip) {
      try {
        console.log(trip)
        const PDFGenerated = await generatePDFContent(trip);
    ...
    async function generatePDFContent(trip: Trip) {
              ...
              roomsContent.push(
                <View key={`room-${roomType}-${room.id}`} wrap={false}>
                  ...
                  {!room.travelers || room.travelers.length === 0 ? (
                    <Text style={styles.emptyRoom}>Nenhum viajante neste quarto.</Text>
                  ) : (
